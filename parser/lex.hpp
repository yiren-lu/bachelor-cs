...
using TokenSequence = std::queue<std::pair<TOKEN, std::string>>;
...
struct LexAutomata {
  ...
  LexAutomata(std::vector<State> state) : state(state) {}
  ...
  std::vector<Token> Run(
    ...
    auto Output = [&](TOKEN type) {
      auto Can = [&](std::pair<TOKEN, std::string> a,
        ...
        if (a.first == TOKEN::ERR)
          return b.first == TOKEN::BLANK || b.first == TOKEN::SINGLE_MARK ||
                 b.first == TOKEN::OPER;
        return true;
      };
      if (tokens.empty())
        ...
        bool primaryFail = false, splited = false;
        if (IsNumberToken(type)) {  //将1和+1的形式拆成1 + 1
          if (IsNumberToken(typ0) || typ0 == TOKEN::ID) {
            if (token[0] == '+' || token[0] == '-') {
              tokens.push_back({line, col, TOKEN::OPER, token.substr(0, 1)});
              tokens.push_back(
                  {line, col + 1, type, token.substr(1, token.length() - 1)});
              splited = true;
            } else
              primaryFail = true;
          }
    ...
    for (char c; (c = stream.get()) != EOF;) {
      if (!InCharSet(c)) {
        ...
        if (s == INIT &&
          ...
          if (state[s].type ==
              TOKEN::NOT_END) {  // 上一个状态不是终止态，则认为发生了错误
            token += c;
            Output(TOKEN::ERR);  //报错
          } else {
            stream.unget();  // 回退一个字符
    ...
    if (token != "")
      Output(state[s].type == TOKEN::NOT_END ? TOKEN::ERR : state[s].type);
    return tokens;
  }

  TokenSequence GenSequence(std::ifstream &stream) {
    auto tokens = Run(stream);
    TokenSequence tok;
    for (auto &[l, r, typ, str] : tokens)
      if (typ != TOKEN::BLANK)
        tok.push({typ, IsNumberToken(typ) ? "num" : str});
    return tok;
  }
} clangLex(  // 自动机的初始化过程
    {        // INIT
     ...
     // ALPHA
     {TOKEN::ID, {{[](char c) -> bool { return isalnum(c) || c == '_'; }, ID}}},
     // SINGLE_MARK
     {TOKEN::SINGLE_MARK, {}},
     // DIV
     {TOKEN::OPER,
      {{Include({'='}), SELF_OPER},
       ...
       {Include({'*'}), DS_COMMA}}},
     // LESS
     {TOKEN::RELOP, {{Include({'='}), LESS_EQUAL}, {Include({'<'}), BIN_OPER}}},
     // GREATER
     {TOKEN::RELOP,
     ...
     // AND
     {TOKEN::OPER, {{Include({'&'}), AAND}, {Include({'='}), SELF_OPER}}},
     // OR
     {TOKEN::OPER, {{Include({'|'}), OOR}, {Include({'='}), SELF_OPER}}},
     // CHAR_QUOTATION
     {TOKEN::NOT_END,
      {{Include({'\\'}), CHAR_DASH}, {Exclude({'\\', '\'', '\n'}), CHAR_PREF}}},
     // CHAR_PREF
     {TOKEN::NOT_END,
      {{Include({'\\'}), CHAR_DASH},
       {Exclude({'\\', '\'', '\n'}), CHAR_PREF},
     ...
     // BASE8
     {TOKEN::INT,
      {{IsBase8, BASE8}, {Include({'U'}), INT_U}, {Include({'L'}), INT_L}}},
     // BASE16
     {TOKEN::INT,
      {{IsBase16, BASE16}, {Include({'U'}), INT_U}, {Include({'L'}), INT_L}}},
     // FLOAT_DOT
     ...
     // FLOAT
     {TOKEN::FLOAT,
      {{isdigit, FLOAT}, {Include({'e'}), FLOAT_E}, {Include({'L'}), FLOAT_L}}},
     // DS_COMMA_END
     {TOKEN::COMMA, {}},
     // INT_UL
     ...
     {TOKEN::INT, {}},
     // FLOAT_E
     {TOKEN::NOT_END, {{isdigit, FL_INT}, {Include({'+', '-'}), FLOAT_E_FLAG}}},
     // INT_ULL
     {TOKEN::INT, {}},
     // FLOAT_E_FLAG
